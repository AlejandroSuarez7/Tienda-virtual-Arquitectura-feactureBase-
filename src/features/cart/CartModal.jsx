...
export default function CartModal({ open, onClose }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ background: '#fff', borderRadius: '8px', padding: '2rem', minWidth: '350px', maxWidth: '90vw', boxShadow: '0 2px 16px #aaa', position: 'relative' }}>
        ...
        {items.length === 0 ? (
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {items.map(item => (
              <li key={item.id} style={{ display: 'flex', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid #eee', paddingBottom: '0.5rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{color:'black'}}>Cantidad: {item.quantity}</div>
                  <div style={{color:'black'}}>Precio: ${item.price}</div>
                </div>
                <button onClick={() => removeFromCart(item.id)} style={{ background: '#e33', color: '#fff', border: 'none', borderRadius: 4, padding: '0.3rem 0.7rem', cursor: 'pointer' }}>Eliminar</button>
              </li>
            ))}
          </ul>
        )}
